#!/usr/bin/env bash
# build.sh — Build fritzbrowser inside a Docker container for openSUSE Leap / Ubuntu.
#
# Usage:
#   ./docker/build.sh [--distro <alias>|all] [--build-type Release|Debug]
#
# Available distro aliases:
#   opensuse-leap-15.6-x86_64    openSUSE Leap 15.6,  x86_64,  Qt5-only (libsecret)
...
#   ubuntu-24.04-x86_64          Ubuntu 24.04,         x86_64,  Qt6-only (libsecret)
...
#        RPM (openSUSE):  fritzbrowser-<version>-<release>.<arch>.rpm
#        DEB (Ubuntu):    fritzbrowser_<version>-<release>_<deb_arch>.deb
...
build_for_distro() {
    ...
    local image="fritzbrowser-builder-${dockerfile_suffix}"
    ...
    local version
    version="$(grep -A3 '^project(fritzbrowser' "${PROJECT_ROOT}/CMakeLists.txt" \
    ...
    if [[ ! -f "${build_dir}/fritzbrowser" ]]; then
        echo "ERROR: Expected binary '${build_dir}/fritzbrowser' not found." >&2
        exit 1
    fi
    cp "${build_dir}/fritzbrowser" "${out_dir}/fritzbrowser"
    echo "      Binary : ${out_dir}/fritzbrowser"
    ...
    local pkg_file=""
    case "${pkg_type}" in
        RPM) pkg_file="$(find "${build_dir}" -maxdepth 1 -name "fritzbrowser-*.rpm" | sort | tail -1)" ;;
        DEB) pkg_file="$(find "${build_dir}" -maxdepth 1 -name "fritzbrowser-*.deb" | sort | tail -1)" ;;
    esac
    ...
    if [[ -z "${pkg_file}" ]]; then
        ...
        local pkg_out=""
        case "${pkg_type}" in
            RPM) pkg_out="${out_dir}/fritzbrowser-${version}-${release}.${arch}.rpm" ;;
            DEB) pkg_out="${out_dir}/fritzbrowser_${version}-${release}_${deb_arch}.deb" ;;
        esac
        cp "${pkg_file}" "${pkg_out}"
